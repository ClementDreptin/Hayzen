...
decltype(Dvar_ForEach) Dvar_ForEach = reinterpret_cast<decltype(Dvar_ForEach)>(0x822A1C40);
...
gclient_s *GetGClient(int clientNum)
{
    ...
    return &reinterpret_cast<gclient_s *>(0x830CBF80)[clientNum];
}

gentity_s *GetEntity(uint32_t entNum)
{
    return &reinterpret_cast<gentity_s *>(0x82F03600)[entNum];
}
